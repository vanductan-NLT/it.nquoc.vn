...
const SEV_GROUP: Record<SeverityCode, { cls: string; label: string }> = {
  P0: { cls: 'text-red-400', label: 'P0 — Khẩn cấp' },
  P1: { cls: 'text-orange-400', label: 'P1 — Quan trọng' },
  P2: { cls: 'text-yellow-400', label: 'P2 — Bình thường' },
  P3: { cls: 'text-green-400', label: 'P3 — Thấp' },
};
...
export function TicketList() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === '/' && e.target === document.body) { e.preventDefault(); searchRef.current?.focus(); }
    };
  ...
  return (
    <div className="w-[380px] min-w-[280px] shrink-0 border-r border-border flex flex-col overflow-hidden">
      <div className="px-3 py-2.5 border-b border-border bg-surface shrink-0">
        <div className="relative mb-2">
          <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-text-disabled text-[12px]">🔍</span>
          <input
            ref={searchRef}
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            className="w-full bg-surface-alt border border-border rounded-lg px-3 py-2 pl-8 text-xs text-text-primary outline-none focus:border-accent placeholder:text-text-disabled font-sans transition-colors"
          ...
          {CHIPS.map(c => (
            <button
              ...
              className={`px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all cursor-pointer ${
                activeFilter === c.key
                  ? 'bg-accent-muted border-accent text-purple-400'
                  : 'bg-surface-alt border-border text-text-secondary hover:border-border-strong'
              }`}
            >
              {c.label}
            </button>
      ...
      <div className="px-3 py-1.5 flex items-center border-b border-border shrink-0">
        <span className="text-[11px] text-text-muted font-medium">
          {loading ? 'Đang tải...' : `${filtered.length} tickets`}
        </span>
      </div>
      ...
      <div className="overflow-y-auto flex-1 custom-scroll">
        {filtered.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-16 px-4 gap-2">
            ...
            <div className="text-xs text-text-muted text-center">Không tìm thấy ticket</div>
            {(activeFilter !== 'all' || searchQuery) && (
              <button onClick={() => { setFilter('all'); setSearchQuery(''); }} className="text-[11px] text-purple-400 cursor-pointer hover:underline mt-1">← Xem tất cả</button>
            )}
          </div>
        )}
        {(['P0', 'P1', 'P2', 'P3'] as SeverityCode[]).map(sev => {
          ...
          return (
            <div key={sev}>
              <div className={`px-3 py-1.5 text-[9px] font-bold tracking-[1.5px] uppercase flex items-center gap-1.5 border-b border-border sticky top-0 z-5 bg-bg ${g.cls}`}>
                {g.label}
                <span className="opacity-60">{groups[sev].length}</span>
              </div>
              {groups[sev].map(t => (
